...
opengv2::SystemBase::SystemBase(TrackingBase::Ptr tracking, MapBase::Ptr map, ViewerBase::Ptr viewer,
                                ...
                                const std::string &strSettingsFile) : tracking(tracking), map(map), featureExtractor(featureExtractor), matcher(matcher), viewer(viewer),
...
void opengv2::SystemBase::evaluation(int sensorId, std::vector<double> &result)
{
    if (!GT_Tws_[sensorId].empty())
    {
             ...
             it2 != map->keyframes().end(); it1 = it2++)
        {
            ...
            error_R.push_back(std::acos(std::min(1., std::max(-1., (((GT_unitQs1s2.conjugate() * unitQs1s2).matrix().trace() - 1.) / 2.)))) * 180 / M_PI);
        ...
        result.clear();
        {
            ...
            std::transform(error_t.begin(), error_t.end(), diff.begin(), [mean](double x)
                           { return x - mean; });
        ...
        }
        {
            ...
            std::transform(error_R.begin(), error_R.end(), diff.begin(), [mean](double x)
                           { return x - mean; });
            double sq_sum = std::inner_product(diff.begin(), diff.end(), diff.begin(), 0.0);
...
void opengv2::SystemBase::saveKeyFrameTrajectoryTUM(int sensorId, const std::string &filename)
{
    ...
    f.close();
    std::cout << std::endl
              << num << " frames trajectory saved!" << std::endl;
}
